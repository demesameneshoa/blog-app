...
RSpec.describe Post, type: :system do
  describe 'user index page' do
    before(:each) do
      ...
      visit users_path
    end    # I can see the username of all other users.
    it 'displays the username of all other users' do
      @users.each do |user|
        expect(page).to have_content(user.name)
      end
    end    # I can see the profile picture for each user.
    it 'displays the profile pictures of the users' do
      @users.each do |user|
        expect(page).to have_css("img[src*='#{user.photo}']")
      end
    end    # I can see the number of posts each user has written.
  ...
  describe 'when a user is clicked' do
    ...
    it 'it should redirects to that users show page' do
      ...
      user = create(:user) # ! so just create a brand new one otherwise if iterates through all users we get 'ambiguous match' error.      
      visit users_path
